    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="sm:rounded-lg">
                            ...
                            @foreach($peopleInProfession as $person)
                                @can('viewPeople', $contentItem)
                                    ...
                                    <a x-data="{ hover: false }" href="{{ route('people.show', $person) }}" wire:navigate
                                        ...
                                        <span @mouseenter="hover = true" @mouseleave="hover = false" class="relative">
                                            {{ $person->name }}
            ...
            @if($contentItem->youtube_embed_url)
                <div class="mt-4">
                    <iframe class="w-full aspect-video" src="{{ $contentItem->youtube_embed_url }}"
            ...
            @if ($contentItem->additionalImages->isNotEmpty())
                <div x-data="{
                    ...
                    <!-- Modal -->
                    <div x-show="imageModal" style="display: none" x-transition
                        ...
                        <div class="max-w-full max-h-full overflow-auto">
                            <img :src="images[currentImageIndex]" alt="Full Image"
            ...
            @if ($contentItem->is_public)
                <div class="mt-4 bottom-6 inset-x-0 text-center">
                    ...
                    <div class="inline-block">
                        <livewire:likes.like-button :likeable="$contentItem" :key="'like-button-content-' . $contentItem->id" />
                    </div>
                    <!-- End Like Button -->
                </div>
            ...
            @can('update', $contentItem)
                <div class="flex justify-between items-center mt-6">
                    <x-cinema-button href="{{ route('content-items.edit', $contentItem) }}" wire:navigate
                        palette="purple">{{ __('content_items/show.edit_button') }}</x-cinema-button>
                    <x-cinema-button wire:click="delete({{ $contentItem->id }})"
                        wire:confirm="{{ __('content_items/show.delete_confirm_message') }}"
                        palette="red">{{ __('content_items/show.delete_button') }}</x-cinema-button>
                </div>
            @endcan
