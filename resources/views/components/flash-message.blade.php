...
<div x-data="{show: true}" x-init="setTimeout(() => show = false, 6000)" x-show="show"
    ...
    <div
        class="glass-card px-6 py-4">
        <div class="flex justify-between items-center">
            <p class="text-lg font-bold">
                {{ session('message') }}
            </p>
            <button @click="show = false"
                class="z-10 font-bold text-4xl text-gray-400 hover:text-gray-600 dark:text-gray-800 dark:hover:text-gray-300 ml-4 hover:cursor-pointer">
                <flux:icon.x-circle  variant="solid" size="12" class="text-rose-500 dark:text-rose-300"/>
            </button>
        </div>
    </div>
</div>
